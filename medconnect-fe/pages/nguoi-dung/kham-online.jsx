...
export default function PatientOnlineExam() {
  ...
  const onMouseDown = (e) => { draggingRef.current = true; startRef.current = { x: e.clientX - aiPos.x, y: e.clientY - aiPos.y }; };
  const onMouseMove = (e) => { if (!draggingRef.current) return; setAiPos((p) => ({ ...p, x: e.clientX - startRef.current.x, y: e.clientY - startRef.current.y })); };
  const onMouseUp = () => {
        ...
        <div className="flex-1 min-w-0 relative bg-black">
          ...
          <div className="absolute left-0 right-0 bottom-0 pb-6 flex items-center justify-center">
            <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md rounded-full px-4 py-3 ring-1 ring-white/20 shadow-lg">
              <Button isIconOnly variant="flat" color={muted?"warning":"default"} onPress={()=>setMuted(!muted)} className="bg-white/10">{muted? <MicOff/> : <Mic/>}</Button>
              <Button isIconOnly variant="flat" color={camOff?"warning":"default"} onPress={()=>setCamOff(!camOff)} className="bg-white/10">{camOff? <VideoOff/> : <Video/>}</Button>
              <Button isIconOnly variant="flat" color="default" className="bg-white/10"><MonitorUp/></Button>
              <Divider orientation="vertical" className="h-8 mx-1 bg-white/30" />
              <Button color="danger" startContent={<PhoneOff/>} className="font-semibold">Rời cuộc gọi</Button>
            </div>
          </div>
        </div>
        ...
        {showChat && (
          <div className="w-[380px] h-full bg-white border-l border-gray-200 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              <Card shadow="none" className="bg-blue-50"><CardBody className="p-3 text-sm">Xin chào bác sĩ.</CardBody></Card>
              <div className="flex justify-end">
                <Card shadow="none" className="bg-gray-50 max-w-[80%]"><CardBody className="p-3 text-sm">Chào bạn, tôi là bác sĩ phụ trách.</CardBody></Card>
              </div>
            </div>
            <Divider/>
