...
const DoctorAppointments = () => {
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            const appointmentDTO = { ...selectedAppt, status: "CONFIRMED" };
            ...
            if (res.ok) {
                ...
                setAppointments(prev =>
                    prev.map(a => (a.appointment_id ?? a.id) === id ? updated : a)
                );
                onClose();
            } else {
    ...
    const handleReject = async () => {
        ...
        try {
            const token = await auth.currentUser.getIdToken();
            const id = selectedAppt.appointment_id ?? selectedAppt.id;
            const appointmentDTO = { ...selectedAppt, status: "DENIED" };
                    ...
                    <Card shadow="sm" className="border-l-4 border-warning">
                        <CardBody className="p-4">
                            ...
                            <p className="text-2xl font-bold text-warning">
                                {appointments.filter(a => (a.status || '').toString().toUpperCase() === 'PENDING').length}
                            </p>
                        </CardBody>
                    </Card>
                    ...
                    <Card shadow="sm" className="border-l-4 border-primary">
                        <CardBody className="p-4">
                            ...
                            <p className="text-2xl font-bold text-primary">
                                {appointments.filter(a => (a.status || '').toString().toUpperCase() === 'CONFIRMED').length}
                            </p>
                        </CardBody>
                    </Card>
                    ...
                    <Card shadow="sm" className="border-l-4 border-success">
                        <CardBody className="p-4">
                            ...
                            <p className="text-2xl font-bold text-success">
                                {appointments.filter(a => (a.status || '').toString().toUpperCase() === 'FINISHED').length}
                            </p>
                        </CardBody>
                    </Card>
                                    ...
                                    {selectedAppt.status && selectedAppt.status.toString().toUpperCase() === 'PENDING' && (
                                        <div className="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded">
                                            <p className="text-sm text-yellow-800">
                                                <strong>Lưu ý:</strong> Lịch hẹn này đang chờ xác nhận.
                                            </p>
                                        </div>
                                    )}
                                </div>
